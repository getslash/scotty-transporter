...
RUN curl https://static.rust-lang.org/dist/rust-1.0.0-x86_64-unknown-linux-gnu.tar.gz | tar xz -C /opt
RUN apt-get install -qqy gem ruby-dev build-essential
RUN gem install fpm
RUN /opt/rust-1.0.0-x86_64-unknown-linux-gnu/install.sh
